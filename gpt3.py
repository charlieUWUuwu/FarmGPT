...
class GPT3ForCausalLM(GPT3PreTrainedModel, ABC):
    ...
    def forward(
        ...
        hidden_states = transformer_outputs[0] # (batch_size, seq_len, embedding_size)
        lm_logits = self.lm_head(hidden_states) # lm_logits:(batch_size, seq_len, vocab_size)
        loss = None
        if labels is not None: # labels : (batch_size, seq_len)，與input_ids相同
            # Shift so that tokens < n predict n
            shift_logits = lm_logits[..., :-1, :].contiguous() # shift_logits:(batch_size, seq_len-1, vocab_size)
